...
static const ChkItem CK_MASKS[] = {
    ...
    { "mask_garo",        "Garo's Mask",            "Reward from a Garo Master.",                                  "Ikana Canyon.", CKI_SPRITE, MSPR_GARO_MASK, CK_SCANEQ, MASKS_BASE, 0x3B, MASKS_LEN },
    ...
    { "mask_greatfairy",  "Great Fairy's Mask",     "Reward from the Great Fairy for collecting all Stray Fairies.",        "Clock Town Great Fairy Fountain.", CKI_SPRITE, MSPR_MASK_GREATFAIRY, CK_SCANEQ, MASKS_BASE, 0x40, MASKS_LEN },
    { "mask_gibdo",       "Gibdo Mask",             "Given by the Gibdos.",                               "Ikana royal crypt.", CKI_SPRITE, MSPR_MASK_GIBDO, CK_SCANEQ, MASKS_BASE, 0x41, MASKS_LEN },
    { "mask_dongero",     "Don Gero's Mask",        "Reward for gathering the frogs.",                      "Woodfall spring.", CKI_SPRITE, MSPR_MASK_DONGERO, CK_SCANEQ, MASKS_BASE, 0x42, MASKS_LEN },
    ...
    { "mask_captain",     "Captain's Hat",          "",                                         "Pirates' Fortress.", CKI_SPRITE, MSPR_MASK_CAPTAIN, CK_SCANEQ, MASKS_BASE, 0x44, MASKS_LEN },
    ...
    { "mask_blast",       "Blast Mask",             "Reward from the bomb shop owner.",                            "West Clock Town.", CKI_SPRITE, MSPR_MASK_BLAST, CK_SCANEQ, MASKS_BASE, 0x47, MASKS_LEN },
...
static const ChkItem CK_HEARTS[] = {
    ...
    { "hp_19", "Business Scrub (Termina Field)", "Buy for 100 rupees.",                 "Near the Astral Observatory, Termina Field.", CKI_SPRITE, MSPR_HEART, CK_MANUAL, 0, 0 },
    ...
    { "hp_49", "Moon: Deku trial",              "Odolwa's dungeon.",                                    "The Moon.", CKI_SPRITE, MSPR_HEART, CK_MANUAL, 0, 0 },
    { "hp_50", "Moon: Goron trial",             "Goht's dungeon.",                                      "The Moon.", CKI_SPRITE, MSPR_HEART, CK_MANUAL, 0, 0 },
    { "hp_51", "Moon: Zora trial",              "Gyorg's dungeon.",                                     "The Moon.", CKI_SPRITE, MSPR_HEART, CK_MANUAL, 0, 0 },
    { "hp_52", "Moon: Link trial",              "Twinmold's dungeon.",                                  "The Moon.", CKI_SPRITE, MSPR_HEART, CK_MANUAL, 0, 0 },
};
...
static const ChkItem CK_SONGS[] = {
    { "song_01", "Song of Time",             "Recovered from Skull Kid.",                    "Clock Tower.", CKI_NOTE, 0, CK_BIT, 0x7763D1, 0x10 },
    { "song_02", "Song of Healing",          "From the Happy Mask Salesman.",                "Clock Tower.", CKI_NOTE, 0, CK_BIT, 0x7763D1, 0x20 },
    ...
    { "song_05", "Song of Soaring",          "From Kaepora Gaebora.",               "Southern Swamp owl statue.", CKI_NOTE, 0, CK_BIT, 0x7763D1, 0x80 },
    ...
    { "song_10", "Elegy of Emptiness",       "From Igos du Ikana.",                   "Ancient Castle of Ikana.", CKI_NOTE, 5, CK_BIT, 0x7763D1, 0x02 },
    { "song_11", "Oath to Order",            "From Odolwa's Giant, after clearing the temple.",           "Woodfall Temple.", CKI_NOTE, 1, CK_BIT, 0x7763D1, 0x04 },
    { "song_12", "Scarecrow's Song",         "Teach it to Pierre.",  "Trading Post / Astral Observatory.", CKI_NOTE, 0, CK_BIT, 0x7763D2, 0x02 },
    { "song_13", "Song of Storms",           "MM3D-EXCLUSIVE. After Flat's eulogy + Iron Knuckle; needs Captain's Hat.", "Beneath the Graveyard.", CKI_NOTE, 4, CK_BIT, 0x7763D2, 0x01 },
};
...
static const ChkItem CK_BOTTLES[] = {
    { "bottle_01", "Kotake's Red Potion",       "After saving Koume.",                              "Woods of Mystery.", CKI_SPRITE, MSPR_B_REDPOTION, CK_MANUAL, 0, 0 },
    ...
    { "bottle_04", "Gold Dust",                 "Win the race after defeating Goht.",                      "Goron Racetrack.", CKI_SPRITE, MSPR_B_GOLDDUST, CK_MANUAL, 0, 0 },
...
static const ChkItem CK_EQUIP[] = {
    ...
    { "eq_gilded",   "Gilded Sword",           "Final sword tier. Needs Gold Dust.",                    "Smithy.", CKI_SPRITE, MSPR_SWORD, CK_BYTEEQ,  0x776352, 0x23 },
    ...
    { "eq_razor",    "Razor Sword",            "100 rupees, wait until morning. Reverts on Song of Time.", "Mountain Smithy.", CKI_SPRITE, MSPR_EQ_RAZOR, CK_MANUAL, 0, 0 },
    ...
    { "eq_quiver2",  "Large Quiver (40)",      "40+ points.",                       "Town Shooting Gallery.", CKI_SPRITE, MSPR_EQ_QUIVER2, CK_MANUAL, 0, 0 },
    { "eq_quiver3",  "Largest Quiver (50)",    "Perfect run.",                     "Swamp Shooting Gallery.", CKI_SPRITE, MSPR_EQ_QUIVER3, CK_MANUAL, 0, 0 },
    { "eq_bombbag1", "Bomb Bag (20)",          "50 rupees.",                                    "Bomb Shop.", CKI_SPRITE, MSPR_EQ_BOMBBAG1, CK_MANUAL, 0, 0 },
    { "eq_bombbag2", "Big Bomb Bag (30)",      "90 rupees, after saving the old lady Night 1.", "Bomb Shop.", CKI_SPRITE, MSPR_EQ_BOMBBAG2, CK_MANUAL, 0, 0 },
    { "eq_bombbag3", "Biggest Bomb Bag (40)",  "Big Bomb Bag + 200 rupees.", "Goron Village Business Scrub.", CKI_SPRITE, MSPR_EQ_BOMBBAG3, CK_MANUAL, 0, 0 },
    { "eq_wallet1",  "Adult Wallet (200)",     "Deposit 200 rupees.",                     "Clock Town Bank.", CKI_SPRITE, MSPR_WALLET, CK_MANUAL, 0, 0 },
    { "eq_wallet2",  "Giant Wallet (500)",     "All 30 Gold Skulltulas (any day in MM3D).", "Oceanside Spider House.", CKI_SPRITE, MSPR_EQ_WALLET2, CK_MANUAL, 0, 0 },
};
